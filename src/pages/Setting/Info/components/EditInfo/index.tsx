    ...
    async customRequest(options) {
      ...
      if (docDir && file) {
        ...
        if (result) {
          ...
          console.log(await docsCtrl.refItem(image?.key)?.delete());
          setImage({
            key: result.key,
  ...
  const columns: ProFormColumnsType<TargetModel>[] = [
    {
      ...
      renderFormItem: () => {
        return (
          <Space>
            {image ? <Image src={image.thumbUrl} preview={{ src: image.url }} /> : ''}
            <Upload {...uploadProps}>
      ...
      onFinish={async (values) => {
        if (editData) {
          ...
          const res = await userCtrl.company.update({
            ...values,
            avatar: image ? JSON.stringify(image) : '',
          });
          if (res) {
          ...
          const success = await userCtrl.user.createCompany(values);
          if (success) {
            message.success('创建单位成功');
            userCtrl.setCurSpace(success.target.id);
          } else {
