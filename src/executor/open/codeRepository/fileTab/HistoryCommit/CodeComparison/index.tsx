...
const CodeComparison: React.FC<IProps> = ({ current, node, title, PRdata }) => {
  ...
  useEffect(() => {
    (async () => {
      if (PRdata) {
        ...
        const data: any = await current.Codecomparison(`/${node.ID || node}`);
        setDiffData(data);
                              ...
                              return (
                                <div
                                  onClick={() => {
                                    ...
                                    console.log(File);
                                    console.log(current);
                                    console.log(diffData.data.SourcePath);
                                  }}>
                                  {/\.(jpg|jpeg|png|gif)$/i.test(File.Name) ? (
                                    <img
                                      src={`${diffData.data.SourcePath}/${File.Name}`}
                                      alt=""
                                      style={{
                                        maxWidth: '100%',
