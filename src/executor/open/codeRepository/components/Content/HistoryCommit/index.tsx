...
interface IProps {
  current: IRepository;
  setpage: (value: React.SetStateAction<React.JSX.Element>) => void;
}
const { Search } = Input;

const HistoryCommit: React.FC<IProps> = ({ current, setpage }) => {
  //分支列表选项
  ...
  const getFiles = async (datauri: string = '') => {
    ...
    if (results.code === 200) {
      ...
      moveElementToFirst();
      const commitList = await current.HistoryCommitList('/master');
  ...
  const onID = (_item: any) => {
    return () => {
      console.log(_item);
      setpage(<CodeComparison current={current} node={_item} />);
    };
  };
  return (
    <>
