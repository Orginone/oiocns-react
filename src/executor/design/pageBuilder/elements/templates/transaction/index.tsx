...
const ViewEntities: React.FC<IProps> = (props) => {
  ...
  const loadData = async (take: number, page: number) => {
    ...
    for (let i = 0; i < props.forms?.length; i++) {
      const formList: XForm[] = await props.ctx?.view?.pageInfo?.directory?.target?.resource?.formColl?.find([props.forms[i].id])
      const formItem = formList?.[0];
      options.userData.push(...(formItem?.options?.dataRange?.labels?.map((item: any) => item.value) || []))
      formItem?.options?.dataRange?.filterExp && options.filter.push(JSON.parse(formItem?.options?.dataRange?.filterExp))
    }
