...
import { IForm, ISpeciesItem, IThingClass } from '@/ts/core';
...
export class FormReadConfig extends ReadConfigImpl<
  FormModel,
  Context,
  FormSheetConfig,
  ExcelConfig<Context>
> {
  ...
  async initContext(): Promise<void> {
    ...
    context.formIndex = {};
    let thingSpecies = this.sheetConfig.species as IThingClass;
    ...
    for (let child of thingSpecies.children) {
      let forms = await (child as IThingClass).loadForms();
      for (let form of forms) {
        context.formIndex[form.code] = form;
        context.formIndex[form.name] = form;
        context.formIndex[form.id] = form;
