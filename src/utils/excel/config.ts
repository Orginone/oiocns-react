      ...
      case SpeciesType.Data:
        readConfigs.push({
          ...
          checkData: async (data: any[]) => {
            for (let item of data) {
              if (!item['名称'] || !item['代码'] || !item['类型'] || !item['定义']) {
                throw new ReadError(
                  '表[' + config.sheetName + ']存在未填写的名称、代码、类型以及定义！',
                );
              }
          ...
          operatingItem: async (item: { [key: string]: string }, context: Context) => {
            let child = species.children.find(
              (child) => child.metadata.code == item['代码'],
            );
            if (child) {
              item['类型 ID'] = child.id;
          ...
          completed: (current: SheetReadConfig, sheets: SheetReadConfig[]) => {
            ...
            species.children.forEach(
              (item) => (typeMap[item.metadata.name] = item.metadata.id),
            );
            for (let sheet of sheets) {
              if (sheet.sheetName != current.sheetName) {
                ...
                if (index != -1) {
                  sheet.data?.forEach(
                    (item) => (item['类型 ID'] = typeMap[item['类型']]),
                  );
                }
              }
            }
        ...
        readConfigs.push({
          ...
          checkData: async (data: any[]) => {
            ...
            for (let item of data) {
              if (
                ...
                throw new ReadError(
                  '表[属性定义]存在未填写的类型、属性名称、属性类型以及属性定义！',
                );
              }
              if (!item['类型 ID']) {
                throw new ReadError(
                  '表[属性定义]未匹配到名称为' + item['类型'] + '的类型！',
                );
              }
              if (item['属性类型'] == '选择型') {
                if (item['枚举字典']) {
                  if (!dictMap[item['枚举字典']]) {
                    throw new ReadError(
                      '表[属性定义]未获取到名称为' + item['枚举字典'] + '的字典！',
                    );
                  }
                  item['字典 ID'] = dictMap[item['枚举字典']];
                } else {
                  throw new ReadError(
                    '表[属性定义]当属性类型为选择型时，必须填写枚举字典！',
                  );
                }
              }
            }
          ...
          operatingItem: async (item: { [key: string]: string }, context: Context) => {
            ...
            let property = properties.find(
              (property) => property.name == item['属性名称'],
            );
            if (property) {
      ...
      case '字典定义':
        readConfigs.push({
          ...
          checkData: async (data: any[]) => {
            for (let item of data) {
              if (
                ...
                throw new ReadError(
                  '表[字典定义]存在未填写的类型、字典名称、字典代码以及备注！',
                );
              }
              if (!item['类型 ID']) {
                throw new ReadError(
                  '表[字典定义]未匹配到名称为' + item['类型'] + '的类型！',
                );
              }
            }
          },
          ...
          completed: async (current: SheetReadConfig, sheets: SheetReadConfig[]) => {
            let dictMap: { [key: string]: string } = {};
            await (
              await species.current.space.loadDicts()
            ).forEach((item) => (dictMap[item.metadata.name] = item.id));
            for (let sheet of sheets) {
              if (sheet.sheetName != current.sheetName) {
                let index = sheet.metaColumns.findIndex(
                  (item) => item.name == '字典名称',
                );
                if (index != -1) {
                  sheet.data?.forEach(
                    (item) => (item['字典 ID'] = dictMap[item['字典名称']]),
                  );
                }
              }
            }
          ...
          checkData: async (data: any[]) => {
            for (let item of data) {
              if (!item['字典名称'] || !item['名称'] || !item['值'] || !item['备注']) {
                throw new ReadError(
                  '表[字典项定义]存在未填写的字典名称、名称、值以及备注！',
                );
              }
              if (!item['字典 ID']) {
                throw new ReadError(
                  '表[字典项定义]未匹配到名称为' + item['字典名称'] + '的字典！',
                );
              }
            }
          },
