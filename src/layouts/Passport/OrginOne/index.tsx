...
const orginOneLayout: React.FC<{ route: IRouteConfig }> = ({ route }) => {
  const sliders: eachSlider[] = [
    ...
    {
      title: '存储',
      content: '提供各类文件、数据、应用的存储空间。具有操作系统级文件和资源管理器功能。',
    },
    {
      title: '设置',
      content:
  ...
  const background = (
    <div style={{ backgroundColor: '#011736', height: '100%', width: '100%' }}>
      <img
        style={{ bottom: 0, position: 'absolute' }}
        src="/img/passport/orginOne/passport_bg.png"
        alt=""></img>
    </div>
  );
  const logo = (
    <div style={{ fontSize: '26px', fontWeight: 700, color: '#fff' }}>OrginOne</div>
  );
  const loginLabel = <div></div>;
  return (
    <BasicLayout
      route={route}
      imgDir={'/img/passport/orginOne/'}
      sliders={sliders}
      logo={logo}
