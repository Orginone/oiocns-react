...
const Index: React.FC<PrivacyPolicyType> = () => {
  ...
  return (
    <div style={{ padding: '20px', height: '100vh', overflowY: 'auto' }}>
      <h3>奥集能 隐私政策</h3>
      <p>本政策仅适用于奥集能产品或服务。最近更新日期：2023年01月01日。</p>
      <p>
      ...
      <ul>
        ...
        <li>
          ...
          Do Not Track，那么我们的所有网站都会尊重您的选择。 （四）APP基础权限说明 类目
          权限 说明 存储 READ_EXTERNAL_STORAGE 从外部存储读取数据，基础数据存储能力 存储
          WRITE_EXTERNAL_STORAGE 写入数据到外部存储，技术数据读取能力 相机 CAMERA
          使用摄像头进行拍照或录制视频，用户上传图片发布信息等
        </li>
        <li>
      ...
      （三）我们仅会在以下情况下，公开披露您的用户信息： 1、获得您明确同意后；
      <br />
      ...
      九、用户注销。账户注销流程，如果您不想继续享受我们提供的服务，您可以联系我们账号专员进行注销账号，如满足条件，将会在7天内完成注销；
      <br />
      十、如何联系我们，如果您对本隐私政策有任何疑问、意见或建议，通过以下方式与我们联系：
      公司名：杭州好好科创有限公司 电子邮件：hhkc205502@163.com 电　　话：+86 0571-96119
    </div>
  );
};
