...
const Storage: React.FC<StorageType> = () => {
  ...
  return <div>
    <div className={cls.commentStyle}>
      <div>
        <Input size="middle" className={cls.inputStyle} placeholder="搜索关键词" prefix={<SearchOutlined />} />
        <span className={cls.muchSelect}>多选</span>
      </div>
      <div className={cls.commentMes}>
        {commentList.map((item,index) => { 
          return <div className={`${cls.allComment} ${index===clickName ? cls.selectAllComment:''}`} onClick={() => { 
            setClickName(index);
      ...
      >
        <h4>应用A</h4>
        <Comment
          avatar={<Avatar size="large" shape="square" src="https://randomuser.me/api/portraits/men/70.jpg" />}
          content={
            <div>
              <div className={cls.allTypeMes}>
                <span className={cls.typeStyle}>类型</span><span className={cls.typeStyle}>类型</span><span className={cls.typeStyle}>类型</span>
              </div>
              <div>
      ...
      <Card hoverable={true} style={{marginBottom:'10px',borderRadius:'10px'}}>
        ...
        <Comment
          content={
            <div>
              <a style={{ color:'rgba(0, 0, 0, 0.45)'}}>共103条勾选<RightOutlined /></a>
              <div>
