     ...
     <Layout className={cls.container}>
      ...
      <Card bordered={false}>
        <Descriptions title={title} column={2}>
          ...
          <Descriptions.Item label="联系方式">{{ user }.user.team.code}</Descriptions.Item>
          <Descriptions.Item label="家庭地址" span={2}>
            No. 18, Wantang Road, Xihu District, Hangzhou, Zhejiang, China
          </Descriptions.Item>
        </Descriptions>
        <Collapse expandIcon={(pro) => {
          if (pro.isActive) {
            return (
              <a
          ...
          return (
            <a style={{ fontSize: '14px', whiteSpace: 'nowrap' }}>
              更多信息<CaretDownOutlined />
            </a>
          );
        }} onChange={value} className={cls['ogo-collapse']} expandIconPosition='right' collapsible="header" defaultActiveKey={['1']} ghost>
          <Panel header={null} key="0" style={{ float: 'right' }}>
            <Descriptions >
              <Descriptions.Item style={{ float: 'left' }} label="UserName">Zhou Maomao</Descriptions.Item>
              <Descriptions.Item style={{ float: 'left' }} label="Telephone">1810000000</Descriptions.Item>
